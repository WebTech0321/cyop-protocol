...
export const ProgressBarStatus = ({value, total, unit, animateInterval = 3, visible = true} : IProgressBarStatus) => {
	...
	return (
		<div className="co-progress-status d-flex justify-content-between">
            ...
            <span className="ms-1 text-desc text-end" style={{minWidth: '90px'}}>{`${value ? Math.round(currentValue * (total || 0) / 100) : ''} ${unit}`}</span>
            <span>]</span>
		</div>
	)
}
